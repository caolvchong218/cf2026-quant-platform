...
**平台 v2.2.0 / 增强版 V4，策略研究保留 V3。** 本地运行的 Python 研究平台：从行情、因子和策略，到成交核对、风险解释与实验记录。
...
主环境102项测试通过，1项可选Qlib引擎测试因未在主环境安装Qlib而跳过；该引擎测试在独立Qlib环境的5项专项测试中实际通过。保留21个V3账本检查与独立环境复算。新分析模块重算V3指标与原证据一致，测试覆盖首日损失、内部缺日、月复利、未恢复回撤、CSV编码/金额/重复、笔记与回执安全保存。
