...
const build=path.join(root,'tmp/ppt-build-v4');
const materials=process.env.MATERIALS_DIR ?? 'reports/platform_v4';
const final=path.join(root,materials,process.env.PPTX_NAME ?? 'CF2026_V4_Presentation.pptx');
...
for(let j=0;j<data.length;j++){
 ...
 if(d.kind==='cover'){
   text(s,'可复现的量化研究\n与实用工作台',73,247,1110,175,58,navy,true);
   text(s,d.items.join('\n'),78,459,1120,135,27,gray);
   text(s,(d.date??'2026年9月21日')+'   /   20分钟课程展示',78,623,1100,36,24,teal);
 }else if(d.screenshot){
   const bytes=await fs.readFile(path.join(root,d.screenshot));
   s.images.add({blob:new Uint8Array(bytes),contentType:'image/png',alt:d.title+'：平台实测界面',fit:'contain',position:{left:58,top:214,width:825,height:438}});
   d.items.slice(0,4).forEach((v,k)=>{text(s,v,914,230+k*100,308,88,25,navy,k===0);});
 }else if(d.chart){
   ...
   const stages=(d.title.includes('年度')||d.title.includes('滚动'))?[['此前三年','标签实现后训练'],['2023–2024','验证并保留选择'],['2025–2026/9','开发后历史复核']]:[['2020–2022','训练模型'],['2023–2024','验证并选择'],['2025–2026/9','冻结选择后评估']];
 ...
 s.speakerNotes.textFrame.setText(`建议用时：${d.seconds}秒\n${d.notes}\n${d.source??'来源：本项目 evidence/research_v3、platform_v4；完整定义与限制见最终报告。'}`);
}
